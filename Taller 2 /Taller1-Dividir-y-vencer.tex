...
y análisis tanto del invariante como de su complejidad. Además contienen los mismos elementos anteriormente
mencionados, pero que estudian el problema de encontrar el prefijo común más largo en una secuencia de palabras.
\end{abstract}
...
En la posición $q$ queda el pivote aleatorio y todos los elementos a su izquierda y derecha, están parcialmente ordenados. Eventualmente, a medida que se generan particiones, en $QuickSortSearchAux$ la variable $q$ tendrá el índice de el valor buscado ($S[q] = v$) y por lo tanto se retornará.
\part{Bono - Prefijo común más largo}
\subsection{Análisis}
El problema se puede resumir, de forma informal, como; encontrar el prefijo común más largo entre una colección de palabras. Se habla de colección de palabras a una secuencia de elementos que están compuestos de caracteres, de modo que $S$ sea la secuencia, $S_i$ un elemento, $n$ la cardinalidad de la secuencia y $m$ el número de 
...
\]
Donde $\mathbb{T}$ es el conjunto de caracteres. Finalmente, se define al prefijo común más largo $S_i'$ como:
\[
S_i'=\left\langle S_{i1},S_{i2},\cdots,S_{im}|\forall x \in S_{i}' \Rightarrow x \in S_{i}\right\rangle
\] 
\subsection{Diseño}
Teniendo en cuenta lo establecido en la etapa de análisis del problema, el <<contrato>> del algoritmo que lo
...
\begin{defn*}
	Salidas:
	\begin{enumerate}
		\item El prefijo común más largo definido como $S_i'=\left\langle S_{i1},S_{i2},\cdots,S_{im}|\forall x \in S_{i}' \Rightarrow x \in S_{i}\right\rangle$
	\end{enumerate}
\end{defn*}
~~~~~
...
\end{algorithm}
El anterior algoritmo requiere de $PrefijoAux$ donde se desarrolla la lógica recursiva que se aprovecha 
del paradigma de dividir-y-vencer para encontrar el prefijo común más largo.
\begin{algorithm}[H]
	\begin{algorithmic}[1]
		...
		\EndProcedure
	\end{algorithmic}
	\caption{Distribución recursiva de las palabras}
\end{algorithm}
Este algoritmo requiere de $Prefijo2$, el cual retorna el prefijo común más largo entre dos palabras.
\begin{algorithm}[H]
	\begin{algorithmic}[1]
		...
		\EndProcedure
	\end{algorithmic}
	\caption{Prefijo común más largo entre dos palabras}
\end{algorithm}
\subsubsection{Complejidad}
Para encontrar el análisis de complejidad es pertinente encontrar, por inspección de código, la complejidad de 
$Prefijo2$ que, por el único For que tiene, resulta tener un orden de complejidad de $O(m)$ donde $m$ es
el tamaño de la menor palabra de la secuencia. Después de esto, teniendo en cuenta la complejidad fuera de la recursión ($O(m)$), la cantidad de llamados 
recursivos ($a = 2$) y el número de divisiones ($b = 2$) se puede establecer:
$$
T(n)=\begin{cases}
O(1),
& \mbox{caso base,}\\
